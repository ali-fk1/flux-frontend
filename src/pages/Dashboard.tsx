  ...
  const handleSocialConnect = async (platform: string) => {
    ...
    try {
      if (platform === "facebook") {
        await triggerFacebookConnect();
      } else if (platform === "instagram") {
        await triggerInstagramConnect();
      }
    ...
    <div className={`flex h-screen bg-background ${darkMode ? "dark" : ""}`}>
      ...
      >
        <div className="p-4 flex items-center justify-between">
          <h1
            className={`font-bold text-xl ${sidebarCollapsed ? "hidden" : "block"}`}
          >
            Flux
          </h1>
        ...
        <nav className="mt-6">
          <ul className="space-y-2">
            ...
            <li>
              <Button
                variant={activeView === "connections" ? "secondary" : "ghost"}
                className={`w-full justify-start ${sidebarCollapsed ? "justify-center px-0" : ""}`}
            ...
            {activeView === "connections" && (
              <div className="grid md:grid-cols-3 gap-6 max-w-4xl mx-auto">
                {socialPlatforms.map((platform) => {
                  ...
                  // Use ConnectXButton for Twitter/X
                  if (platform.id === "twitter") {
                    return (
                      <Card
                        key={platform.id}
                        className="hover:shadow-lg transition-all duration-300 hover:scale-105 border-2 hover:border-blue-200 bg-white/80 backdrop-blur-sm"
                      >
                        <CardHeader className="text-center">
                          <div
                            className={`w-16 h-16 rounded-full ${platform.color} flex items-center justify-center mx-auto mb-4 shadow-lg`}
                          >
                  ...
                  // Other platforms use the original button
                  return (
                    <Card
                      key={platform.id}
                      className="hover:shadow-lg transition-all duration-300 hover:scale-105 border-2 hover:border-blue-200 bg-white/80 backdrop-blur-sm"
                    >
                      ...
                      <CardContent>
                        <Button
                          ...
                          className={`w-full ${isConnected ? "bg-green-500 hover:bg-green-600" : platform.color} text-white shadow-md hover:shadow-lg transition-all duration-200`}
                        >
                          {isLoading ? (
                            <>
